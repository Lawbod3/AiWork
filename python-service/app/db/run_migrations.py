...
def _get_sqlite_path(database_url: str) -> str:
    """Extract file path from SQLite URL like sqlite:///./briefing.db"""
...
def _applied_migrations_sqlite(conn: sqlite3.Connection) -> set[str]:
    cursor = conn.execute('SELECT filename FROM schema_migrations;')
    rows = cursor.fetchall()
    return {row[0] for row in rows}


def _apply_migration_sqlite(conn: sqlite3.Connection, filename: str, sql: str) -> None:
...
def _revert_migration_sqlite(conn: sqlite3.Connection, filename: str, sql: str) -> None:
    conn.executescript(sql)
    conn.execute(
...
def _apply_pending_migrations_sqlite(conn: sqlite3.Connection) -> int:
    ...
    for filename in migration_files:
        ...
        _apply_migration_sqlite(conn, filename, sql)
        applied_count += 1

    return applied_count


def _latest_applied_migrations_sqlite(conn: sqlite3.Connection, steps: int) -> list[str]:
    cursor = conn.execute(
        '''
        SELECT filename
...
def _rollback_migrations_sqlite(conn: sqlite3.Connection, steps: int) -> int:
    filenames = _latest_applied_migrations_sqlite(conn, steps)
    if not filenames:
    ...
    for filename in filenames:
        ...
        _revert_migration_sqlite(conn, filename, sql)
        reverted_count += 1
...
def main() -> None:
    ...
    if _is_sqlite(database_url):
        ...
        try:
            ...
            if args.command == 'up':
                count = _apply_pending_migrations_sqlite(conn)
                print(f'Migrations complete. Applied: {count}.')
                return

            count = _rollback_migrations_sqlite(conn, args.steps)
